    ...
    <div className="flex flex-col gap-6 w-full max-w-lg animate-fade-in pb-20 px-4 sm:px-0">
      {isST && lastEvent && lastEvent.type === 'slayer_shot' && (Date.now() - lastEvent.timestamp < 60000) && (
        <div className="bg-rose-600 text-white p-6 rounded-[2.5rem] shadow-2xl animate-bounce text-center space-y-4">
           <div><p className="text-[10px] font-black uppercase opacity-80 mb-2 tracking-widest">학살자 사격 발동!</p><p className="text-2xl font-black">{lastEvent.actorName} {'->'} {lastEvent.targetName}</p></div>
           <div className="flex gap-3">
              <Button onClick={() => handleResolveSlayer(true)} variant="primary" className="flex-1 bg-white text-rose-600 font-black h-16">악마 사망</Button>
              <Button onClick={() => handleResolveSlayer(false)} variant="secondary" className="flex-1 bg-rose-900 text-white border-transparent h-16">불발</Button>
           </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/80 p-6 rounded-2xl border border-slate-800 backdrop-blur flex justify-between items-center shadow-lg">
        ...
        {roomState.executionTargetUid && (
          <div className="text-right">
            <p className="text-[10px] text-rose-500 font-black uppercase tracking-widest">처형 후보</p>
        ...
        <div className="bg-slate-950 p-8 rounded-[2.5rem] border border-sky-500/30 text-center relative overflow-hidden shadow-2xl animate-fade-in">
          ...
          <div className="flex justify-center gap-10 mb-10">
            <div className="flex flex-col items-center bg-slate-900/60 p-6 rounded-3xl border border-sky-500/20 w-32 shadow-inner">
              ...
              <span className="text-[10px] font-black text-slate-500 uppercase">찬성 (최소: {majorityNeeded})</span>
            </div>
            <div className="flex flex-col items-center bg-slate-900/60 p-6 rounded-3xl border border-rose-500/20 w-32 shadow-inner">
              ...
              <span className="text-[10px] font-black text-slate-500 uppercase">반대</span>
            </div>
          </div>
          {!isST ? (
            <div className="flex gap-4">
              <Button onClick={() => handleVote(true)} variant={voters[user.uid] === true ? "primary" : "secondary"} size="lg" className="flex-1 font-black h-20 text-xl shadow-xl uppercase">찬성</Button>
              <Button onClick={() => handleVote(false)} variant={voters[user.uid] === false ? "danger" : "secondary"} size="lg" className="flex-1 font-black h-20 text-xl shadow-xl uppercase">반대</Button>
            </div>
          ) : (
            <div className="flex flex-col gap-3">
               ...
               <Button onClick={handleCancelNomination} variant="ghost" className="w-full text-xs text-slate-500 uppercase tracking-widest font-black underline underline-offset-8 decoration-slate-800">지목 취소</Button>
            </div>
          )}
        </div>
      )}
          ...
          <button onClick={() => setShowPlayerLog(!showPlayerLogLocal)} className="w-full p-8 flex items-center justify-between hover:bg-slate-800/50 transition-all group">
            <div className="flex items-center gap-5">
               ...
               <div className="text-left">
                  ...
                  <span className="text-lg font-black text-slate-200 tracking-tight uppercase font-serif">내 정체 및 정보 기록</span>
               </div>
            </div>
               ...
               {playerSecret?.alignment === 'evil' && playerSecret.evilTeamInfo && (
                  <div className="p-6 bg-rose-950/20 border border-rose-500/20 rounded-3xl space-y-6 shadow-xl text-center">
                     ...
                     <div className="grid grid-cols-1 gap-6">
                        <div><span className="block text-[10px] text-slate-500 font-black uppercase mb-1">악마(Demon)</span><span className="text-xl text-rose-400 font-black uppercase tracking-tight italic underline decoration-rose-500/20">{playerSecret.evilTeamInfo.demonName}</span></div>
                        <div><span className="block text-[10px] text-slate-500 font-black uppercase mb-1">하수인(Minions)</span><span className="text-base text-white font-bold leading-tight uppercase tracking-tight">{playerSecret.evilTeamInfo.minionNames.join(', ')}</span></div>
                     ...
                     {playerSecret?.messageHistory && playerSecret.messageHistory.length > 0 ? (
                        playerSecret.messageHistory.map((msg, i) => (
                          <div key={i} className="p-5 bg-slate-900/80 rounded-2xl border border-slate-800 shadow-lg text-base text-slate-300 italic font-serif leading-relaxed relative overflow-hidden group">
                             ...
                             <div className="flex justify-between items-center mb-3"><span className="text-[10px] font-black text-sky-500/60 uppercase tracking-[0.2em]">기록 #{i + 1}</span><span className="text-[8px] font-bold text-slate-700 font-mono text-xs">일시: 밤 {i + 1}</span></div>
                             <p className="pl-2">"{msg}"</p>
                          </div>
                        ))
                     ) : (<div className="py-12 px-6 text-center border-2 border-dashed border-slate-800 rounded-[2.5rem] bg-slate-950/20 shadow-inner"><p className="text-slate-700 font-black uppercase tracking-widest text-xs font-serif">복구된 기록이 없습니다.</p></div>)}
                  </div>
               </div>
            </div>
          )}
      ...
      {!isST && playerSecret?.character === 'slayer' && !roomState.players[user.uid]?.isDead && (
         <div className="bg-rose-950/30 p-8 rounded-[3rem] border border-rose-500/30 text-center shadow-2xl mt-4 space-y-6 relative overflow-hidden font-serif">
            <div className="absolute top-0 left-0 w-full h-1 bg-rose-500/20 animate-pulse"></div>
            <p className="text-sm text-rose-500 font-black uppercase tracking-[0.4em] mb-2">학살자의 사격 집행</p>
            <div className="grid grid-cols-1 gap-2">
               {players.filter(p => !p.isDead && p.uid !== user.uid).map(p => (
                 <Button key={p.uid} onClick={() => handleSlayerShot(p.uid)} variant="danger" className="w-full font-black uppercase tracking-widest h-14 text-base shadow-xl border-transparent">사격: {p.name}</Button>
               ))}
            </div>
         </div>
      )}
      ...
      {isST && (
        <div className="bg-slate-900/95 p-8 rounded-[3rem] border border-slate-800 backdrop-blur shadow-[0_30px_60px_rgba(0,0,0,0.5)] mt-8 relative overflow-hidden">
           ...
           <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.3em] mb-8 border-b border-slate-800 pb-5 flex justify-between items-center">
              <span>스토리텔러 전용 관리 데스크</span>
              {roomState.executionTargetUid && <span className="bg-rose-600 text-white px-3 py-1 rounded-full text-[10px] animate-pulse italic font-black uppercase tracking-tighter">처형 후보: {roomState.players[roomState.executionTargetUid]?.name}</span>}
           </h3>
           {!isVoting ? (
             <div className="space-y-10">
                      ...
                      return (
                        <div key={target.uid} className={`bg-slate-950/80 p-5 rounded-[2rem] border transition-all ${isTargetUsed ? 'border-slate-900 opacity-30 grayscale' : 'border-slate-800 shadow-xl hover:border-slate-600'}`}>
                           <div className="flex justify-between items-center mb-4">
                              <span className="text-base font-black text-sky-500 uppercase tracking-tighter italic">지목 대상: {target.name}</span>
                              {isTargetUsed && <span className="text-[9px] bg-slate-800 text-slate-500 px-3 py-1 rounded-full uppercase font-black">이미 지목됨</span>}
                           </div>
                           <div className="flex flex-col gap-3">
                              <p className="text-[10px] text-slate-600 font-black uppercase ml-1 tracking-[0.2em]">지목자 선택</p>
                ...
                <Button onClick={finalizeDay} variant="danger" size="lg" className="w-full mt-6 font-black uppercase tracking-[0.3em] h-20 shadow-2xl border-transparent text-xl shadow-rose-950/40 hover:shadow-rose-600/20 active:scale-[0.98]">
                   {roomState.executionTargetUid ? '처형 집행 및 아침으로' : '처형 없이 낮 종료'}
                </Button>
             </div>
           ) : (
             <div className="flex flex-col items-center justify-center py-24 gap-8">
                ...
                <p className="text-sm font-black text-sky-500 uppercase tracking-[0.5em] animate-pulse">평의회 투표 진행 중...</p>
             </div>
           )}
        </div>
      )}
